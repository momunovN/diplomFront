...
export default function BookingHistory() {
  ...
  if (!user) {
    return (
      <div className="container-centered py-32 text-center">
        ...
        <p className="text-2xl text-text-secondary mb-12">Войдите в аккаунт, чтобы увидеть свои билеты</p>
        <Link 
          to="/" 
  ...
  return (
    <div className="container-centered py-12">
      <h1 className="text-5xl md:text-6xl font-bold mb-16 text-center">Мои билеты</h1>

      {bookings.length === 0 ? (
        <div className="text-center py-32">
          <div className="text-6xl mb-8">🎬</div>
          <p className="text-3xl text-text-secondary mb-12">У вас пока нет забронированных билетов</p>
          <Link 
            to="/movies" 
            className="inline-block bg-primary px-12 py-5 rounded-xl text-2xl font-bold hover:bg-primary-dark transition transform hover:scale-105 shadow-2xl"
          >
            ...
            >
              ...
              <div className="space-y-4 text-lg">
                <div className="flex justify-between">
                  <span className="font-semibold text-text-secondary">Имя:</span>
                  <span>{booking.name || user.email}</span>
                </div>
                <div className="flex justify-between">
                  <span className="font-semibold text-text-secondary">Билетов:</span>
                  <span className="text-primary font-bold">{booking.seats}</span>
                </div>
                <div className="flex justify-between">
                  <span className="font-semibold text-text-secondary">Дата:</span>
                  <span>{new Date(booking.createdAt).toLocaleDateString('ru-RU')}</span>
                </div>
                <div className="flex justify-between">
                  <span className="font-semibold text-text-secondary">Время:</span>
                  <span>{new Date(booking.createdAt).toLocaleTimeString('ru-RU', { hour: '2-digit', minute: '2-digit' })}</span>
                </div>
              </div>

              <div className="mt-8 pt-6 border-t border-gray-700 text-center">
                <p className="text-sm text-text-secondary">
                  ID брони: <span className="font-mono text-primary">{booking._id}</span>
                </p>
              </div>
            </div>
          ))}
